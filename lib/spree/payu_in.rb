...
module Spree::PayuIn
  ...
  def gateway_callback
    ...
    if params[:status] == 'failed'
      ...
      flash.now[:notice] = "Payment processing failed. Please choose a different payment option, or try again."
      @gateway = payment_method.provider
      render 'checkout/failed'
    elsif params[:status] == 'canceled'
      flash.now[:notice] = "You cancelled payment. Please choose a different payment option, or try again."
      payment_method = PaymentMethod.find(params['udf1'])
      @gateway = payment_method.provider
      render 'checkout/failed'
    else
